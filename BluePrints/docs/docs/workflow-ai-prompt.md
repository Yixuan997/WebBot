...
- `workflow`: 节点数组（建议按阅读顺序排列，真实执行以跳转字段为准）

**节点结构**：
- `id`: 唯一标识，start 节点固定为 `"start"`，其他用 `"node_1"`, `"node_2"` 等
- `type`: 节点类型（见下方节点列表）
- `config`: 节点配置

**执行路径规则（重要）**：
- 节点通过 `next_node / true_branch / false_branch / loop_body` 指定执行路径
- 没有后续节点时流程结束，不需要也不允许生成 `end` 节点
- 不按数组“下一个节点”自动兜底；需要继续执行时必须配置对应跳转字段
- `start.next_node` 必填
- 普通节点的 `next_node` 可选：需要继续就填，作为终点就留空
- `condition.true_branch / false_branch` 可选：某个分支没有连线时，该分支结束
- `keyword_trigger.next_node` 建议填写，未匹配会自动中断当前工作流
- `foreach.loop_body` 必填，`next_node` 可选；循环完成后无 `next_node` 则结束当前路径
## 可用内置变量
...
- `match_type`: string - 匹配类型
  ...
  - `equals`: 完全匹配
  - `starts_with`: 开头匹配
- `next_node`: string - 匹配成功后跳转节点ID（建议填写，留空则匹配后结束）
...
- `true_branch`: string - 满足条件跳转的节点ID（可选，留空则该分支结束）
- `false_branch`: string - 不满足条件跳转的节点ID（可选，留空则该分支结束）
...
- `next_node`: string - 执行后跳转到的节点ID（可选，留空则结束当前路径）
...
- `next_node`: string - 循环结束后跳转的节点ID（可选，留空则结束当前路径）
...
```json
{
  ...
  "workflow": [
    ...
    {"id": "node_3", "type": "foreach", "config": {"list_variable": "response_json.data", "item_variable": "group", "loop_body": "node_4", "delay": "0.5"}},
...
- `next_node` 指定循环结束后跳转的节点；留空时循环完成后结束
- 循环体终点不需要手动连回 foreach；当前迭代结束后会自动返回 foreach 继续下一轮
- `delay` 可以防止发送过快被风控
...
```json
{
  ...
  "workflow": [
    ...
    {"id": "node_2", "type": "foreach", "config": {"list_variable": "response_json.data", "item_variable": "group", "loop_body": "node_3", "delay": "0.5"}},
    ...
    {"id": "node_4", "type": "condition", "config": {
      ...
      "condition_type": "equals",
      "compare_value": "True",
      "true_branch": "node_skip",
      "false_branch": "node_5",
      "stop_after_branch": true
    }},
    {"id": "node_skip", "type": "comment", "config": {"comment": "当前群在跳过名单中，本轮结束"}},
...
**循环中条件节点说明**：
- 循环分支终点可以不配置下游；当前迭代结束后会返回 foreach 进入下一轮
...
1. **必须包含 start 节点，不要生成额外的终止节点**
2. **节点ID唯一**：start, node_1, node_2, ...
3. **变量引用使用双花括号**：`{{变量名}}`
4. **JSON字符串中的引号需要转义**
5. **布尔值在 JSON 模板数据中使用小写**：true/false
6. **关键词用换行符分隔**：`关键词1\n关键词2`
7. **跳转字段执行（强制）**：需要继续执行时必须配置执行路径（如 `next_node` / `true_branch+false_branch` / `loop_body`），否则结束当前路径
8. **禁止旧格式**：严禁输出仅依赖数组顺序的“无连线配置”节点
9. **start/condition/foreach/keyword_trigger 约束**：`start.next_node` 必填，`condition` 只填写实际需要继续的分支，`foreach.loop_body` 必填且 `next_node` 可选，`keyword_trigger` 建议填写 `next_node`
10. **定时工作流需在节点中指定发送目标**
