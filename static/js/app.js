...
function showAlert(type, message) {
    ...
    alert.className = `alert alert-important alert-${type} alert-dismissible`;
    alert.style.cssText = 'position: fixed; top: 1rem; right: 1rem; z-index: 1050; max-width: 400px;';

    // 根据类型选择图标
    const icons = {
        success: '<path d="M5 12l5 5l10 -10"/>',
        info: '<circle cx="12" cy="12" r="9"/><line x1="12" x2="12.01" y1="8" y2="8"/><polyline points="11 12 12 12 12 16 13 16"/>',
        ...
        danger: '<circle cx="12" cy="12" r="9"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/>'
    };
    const iconPath = icons[type] || icons.info;
    const iconSvg = `<svg class="icon alert-icon" fill="none" height="24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" width="24">${iconPath}</svg>`;
...
/**
 * 显示Toast提示（用于AJAX请求后的即时反馈，与 showAlert 功能相同）
 * @param {string} message - 提示消息
 * @param {string} type - 提示类型: success, info, warning, danger, error
 */
